...
const functionMap = {
  send_follow_request: printEvent,
  send_unfollow_request: printEvent,
};
...
export function addMessageListener(eventType, callback) {
  if (!eventHandlers[eventType]) {
...
export function removeMessageListener(eventType, callback) {
  if (eventHandlers[eventType]) {
